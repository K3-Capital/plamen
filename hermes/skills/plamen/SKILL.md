...
1. Display: "Plamen Web3 Security Auditor -- Hermes/OpenAI Codex Runtime"
...
```powershell
# PowerShell
& $PY {PLAMEN_HOME}/hooks/phase_gate.py --init "$SCRATCHPAD" $MODE "$PROJECT_ROOT"
```
```bash
# Bash
$PY {PLAMEN_HOME}/hooks/phase_gate.py --init "$SCRATCHPAD" "$MODE" "$PROJECT_ROOT"
...
Read `{PLAMEN_HOME}/hooks/phase_manifest.json` for the phase ordering and
...
`{PLAMEN_HOME}/prompts/{LANGUAGE}/phase1-recon-prompt.md`
...
- Role: `{PLAMEN_HOME}/codex/agents/recon.toml` (with task subset)
...
Spawn breadth agents in batches of max 6 (from `{PLAMEN_HOME}/codex/agents/breadth.toml`):
...
Spawn the `inventory` agent (from `{PLAMEN_HOME}/codex/agents/inventory.toml`):
...
If MODE is thorough:
- Read `{PLAMEN_HOME}/rules/phase3b-rescan-prompt.md` for re-scan methodology
- Spawn 2-3 `rescan` agents (from `{PLAMEN_HOME}/codex/agents/rescan.toml`) with exclusion list
- Then spawn `per-contract` agents (from `{PLAMEN_HOME}/codex/agents/per-contract.toml`),
...
- Spawn `semantic-invariant` agent (from `{PLAMEN_HOME}/codex/agents/semantic-invariant.toml`)
...
Read `{PLAMEN_HOME}/rules/phase4-confidence-scoring.md` for the full process.
...
Read `{PLAMEN_HOME}/rules/phase4c-chain-prompt.md` for prompts.

### Phase 5: Verification

Spawn `verifier` agents in batches of 6 for each hypothesis batch:
- Read `{PLAMEN_HOME}/rules/phase5-poc-execution.md` for PoC rules
...
Spawn report agents sequentially per `{PLAMEN_HOME}/rules/phase6-report-prompts.md`:
...
```bash
python3 {PLAMEN_HOME}/hooks/phase_gate.py --stop
```
